...
module.exports = class PetController {
    ...
    static async getAllUserAdoptions(req, res) {
        ...
        const pets = await Pet.find({ 'dopter._id': user._id }).sort('-createAt')
    ...
    static async removePetById (req, res) {
        ...
        //check if logger user registered the pet
        const token = getToken(req)
        const user = await getUserByToken(token)
    ...
    static async schedule (req, res) {
        ...
        res.status(200).json({
            message: `A visita já foi agendada com sucesso, entre em contato com ${pet.user.name} pelo telefone ${pet.user.phone}`
        })
    }
    ...
    static async concludeAdoption (req, res) {
        ...
        await Pet.findByIdAndUpdate(id, pet)
        res.status(200).json({ message: 'Parabéns o ciclo da adoção foi finalizado com sucesso!!'})
    }
}